"""
KAGGLE GPU RUN 7 — THE CROSSOVER CONTROL. Fixed physical volume.

Prior-art verdict: the localization crossover is new-for-FP ONLY IF it is coupling-driven
rather than a physical-volume effect (Sternbeck hep-lat/0510109 saw FP-mode localization
and attributed it to volume; our L=6 scan confounds the two exactly as their data did).

PART N  IPR*V and f_IR of the lowest deflated FP mode at MATCHED physical size
        ell ~ 3.1-3.5 in 1/sqrt(sigma) units: (2.2,L6) (2.4,L8) (2.6,L10) (2.8,L12).
         (N1) IPR falls AND f_IR rises with beta AT FIXED ell -> coupling-driven;
              the crossover is claimable as new-for-FP against the mandatory citations.
         (N2) flat in beta at fixed ell -> volume effect; RETRACT the crossover finding
              and reframe as higher-statistics confirmation of Sternbeck.
Gates: G1 plaquette anchor, G3 deflated free-field. ~2-3 h. Writes results7.json.
"""
import json
import time
import numpy as np
...
def psi_min(lat, b, h=1e-4):
    """Lowest deflated FP eigenvector of config b: (val, psi), psi (V,3) normalized."""
    Ub = lat.U[b:b+1]
    sub = Lat.__new__(Lat)
    ...
    A = LinearOperator((3 * V, 3 * V), matvec=mv, dtype=np.float64)
    val, vec = eigsh(A, k=1, which='SA', tol=1e-7, maxiter=50000)
    p = proj(vec[:, 0])
    p /= np.linalg.norm(p)
    return float(val[0]), p.reshape(V, 3)


def diagnostics(L, psi):
    V = L ** 4
    P = (psi ** 2).sum(axis=1)
    iprV = float(V * (P ** 2).sum())
    ...
    return iprV, float(w[kk == 1].sum()), float(w[kk == 0].sum())
...
if __name__ == "__main__":
    ...
    # a*sqrt(sigma) from chi33 (L=16, run 6): 2.2: 0.561, 2.4: 0.434, 2.6: 0.336, 2.8: 0.260
    PLAN = ([(2.2, 6, 10, 3.37), (2.4, 8, 10, 3.47), (2.6, 10, 10, 3.36),
             (2.8, 12, 8, 3.12)] if not FAST else [(2.4, 4, 2, 1.7), (2.8, 6, 2, 1.6)])
    print("\n=== PART N: crossover diagnostics at matched ell ===", flush=True)
    for beta, L, n, ell in PLAN:
        lat = Lat(L, n)
        ...
        for b in range(n):
            try:
                ...
                continue
            iprV, f_ir, f0 = diagnostics(L, psi)
            assert f0 < 1e-4, "deflation leakage"
            ip.append(iprV)
            fi.append(f_ir)
            lm.append(val)
        results["partN"][f"{beta},{L}"] = {
            "ell": ell,
            "iprV": [float(np.mean(ip)),
                     float(np.std(ip, ddof=1) / np.sqrt(len(ip)))],
            "f_ir": [float(np.mean(fi)),
                     float(np.std(fi, ddof=1) / np.sqrt(len(fi)))],
            "lam": float(np.mean(lm)), "n": len(ip)}
        print(f"  beta={beta} L={L} ell={ell}: IPR*V = {np.mean(ip):.3f}"
              f"+-{np.std(ip, ddof=1)/np.sqrt(len(ip)):.3f}"
              f"   f_IR = {np.mean(fi):.3f}+-{np.std(fi, ddof=1)/np.sqrt(len(fi)):.3f}"
              f"   ({time.time()-t0:.0f}s)", flush=True)

    print("\n  known L=6 beta-scan (CONFOUNDED): IPR*V 8.7 -> 1.2, f_IR 0.36 -> 0.95")
    print("  N1 same trends AT FIXED ell -> coupling-driven; crossover claimable")
    print("  N2 flat at fixed ell        -> volume effect; RETRACT the finding")

    with open("results7.json", "w") as f:
        json.dump(results, f, indent=1)
    print(f"\nwrote results7.json   total {time.time() - t0:.0f}s")
